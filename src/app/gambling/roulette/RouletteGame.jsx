...
export default function RouletteGame({ credits, pixels, points }) {
  ...
  const spin = async () => {
    if (spinning || cooldown > 0 || totalBet < 1 || Object.keys(bets).length === 0) return;
    ...
    try {
      ...
      if (res.status === 429 || data.error?.includes('Too fast') || data.error?.includes('Too many')) {
        ...
        triggerCooldown(wait);
        setMessage({ text: `Too fast! Please wait ${wait}s.`, type: 'error' });
        setSpinning(false);
        return;
      }
      setTimeout(() => {
    ...
    const chipImages = {
        5: '/images/gambling/bet_5.png',
        10: '/images/gambling/bet_10.png',
        25: '/images/gambling/bet_25.png',
        50: '/images/gambling/bet_50.png',
        250: '/images/gambling/bet_250.png',
        500: '/images/gambling/bet_500.png',
    };
  ...
  return (
    <div>
      <div className="panel no-hover" style={{ padding: 16, display: 'flex', alignItems: 'center', gap: 20, marginBottom: 20 }}>
              <div style={{ fontSize: 18, fontWeight: 800 }}><img style={{ display: 'inline-block', marginRight: '10px', marginLeft: '-5px' }} src="/images/gambling/roulette_icon.png"></img>Roulette</div>
        ...
        <div>
                          ...
                          {[5, 10, 25, 50, 250, 500].map(v => (
                              <button
                                  ...
                                  style={{
                                      ...
                                      padding: 0,
                                      borderRadius: '50%',
                                      border: chipSize === v ? '2px solid #fff' : '2px solid transparent',
                                      background: 'transparent',
                                      cursor: 'pointer',
                                      opacity: spinning ? 0.5 : 1,
                                      transition: 'all 0.15s ease',
                                      transform: chipSize === v ? 'scale(1)' : 'scale(1)',
                                  }}
                              >
                                  <img
                                      src={chipImages[v]}
          ...
          <div className="panel no-hover" style={{ padding: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <button onClick={spin} disabled={spinning || cooldown > 0 || totalBet < 1 || totalBet > balance} className="btn-enterhotel" style={{ fontSize: 13, opacity: (spinning || cooldown > 0) ? 0.5 : 1 }}>{spinning ? 'Spinning...' : cooldown > 0 ? `Wait ${cooldown}s` : 'Spin!'}</button>
            </div>
          </div>
        </div>
