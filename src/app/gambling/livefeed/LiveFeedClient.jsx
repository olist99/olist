...
const GAME_COLORS = { roulette: '#EF5856', coinflip: '#f5c842', 'coinflip-battle': '#e74c8b', blackjack: '#34bd59' };
const GAME_LABELS = { roulette: 'Roulette', coinflip: 'Coin Toss', 'coinflip-battle': 'Battle', blackjack: 'Blackjack' };
...
export default function LiveFeedClient() {
  ...
  const loadFeed = async (initial = false) => {
    try {
      const since = initial ? 0 : lastId.current;
      const res = await fetch(`/api/livefeed?type=feed&since=${since}`);
      const data = await res.json();
      if (data.feed?.length) {
        if (initial) { setFeed(data.feed); } else {
          setFeed(prev => {
            const newIds = new Set(data.feed.map(f => f.id));
            const merged = [...data.feed, ...prev.filter(f => !newIds.has(f.id))];
            return merged.slice(0, 100);
          });
        }
        lastId.current = Math.max(...data.feed.map(f => f.id));
      }
    } catch {}
  };
  ...
  useEffect(() => { loadFeed(true); const t = setInterval(() => loadFeed(false), 3000); return () => clearInterval(t); }, []);
      ...
      {tab === 'feed' ? (
        <div>
          <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
            {['all', 'roulette', 'coinflip', 'coinflip-battle', 'blackjack'].map(f => (
              <button key={f} onClick={() => setFilter(f)} style={{ padding: '6px 14px', borderRadius: 'var(--radius)', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 11, fontWeight: 700, background: filter === f ? 'var(--green)' : 'var(--panel-inner)', color: filter === f ? '#000' : 'var(--text-secondary)' }}>
          ...
          <div className="panel no-hover" style={{ padding: 0 }}>
            {filtered.length === 0 ? <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)', fontSize: 12 }}>No activity yet.</div> :
              filtered.map(item => (
        ...
        <div>
          <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
            {[['all','All Time'],['month','This Month'],['week','This Week'],['today','Today']].map(([p, l]) => (
              <button key={p} onClick={() => setPeriod(p)} style={{ padding: '6px 14px', borderRadius: 'var(--radius)', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 11, fontWeight: 700, background: period === p ? 'var(--green)' : 'var(--panel-inner)', color: period === p ? '#000' : 'var(--text-secondary)' }}>{l}</button>
            ))}
          </div>
