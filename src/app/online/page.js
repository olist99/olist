...
import { formatNumber, RANK_COLORS, RANK_ICONS } from '@/lib/utils';
...
export default async function OnlinePage({ searchParams }) {
  ...
  const totalOnline = await queryScalar(
    ...
    <div className="animate-fade-up">
      <div className="title-header mb-4">
        ...
        <p className="text-xs text-text-secondary mt-0.5">
          See who&apos;s currently in the hotel
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5 max-md:grid-cols-1">
        {[
          ...
          <div key={s.label} className="card p-4 flex items-center gap-4">
            <img src={s.icon} alt="" style={{ width: 32, height: 32, imageRendering: 'pixelated' }} />
            <div>
      ...
      <div className="flex justify-between items-start mb-5 gap-4 flex-wrap">
        <div className="flex gap-2 items-center flex-wrap">
          {sortOptions.map(o => (
            <Link
              key={o.key}
              href={`/online?sort=${o.key}&q=${encodeURIComponent(search)}`}
              className={`btn btn-sm no-underline ${
                sort === o.key ? 'btn-primary' : 'btn-secondary'
              }`}
            >
              {o.label}
            </Link>
        ...
        <form method="GET" action="/online" className="flex gap-1.5">
          ...
          <input
            type="text"
            name="q"
            className="input !w-44 !py-1.5 !px-3 !text-xs"
        ...
        {users.map(u => {
          ...
          return (
            ...
            >
              ...
              <div className="text-sm font-bold text-text-primary group-hover:text-accent truncate">
                {u.username}
              </div>

              <div className="text-[11px] mt-0.5" style={{ color: rankColor }}>
                <img
                  src={RANK_ICONS[u.rank] || '/images/rank-member.png'}
                  alt=""
      ...
      {users.length === 0 && (
        <div className="card p-16 text-center text-text-muted">
          {search
            ? `No online users matching "${search}".`
      ...
      {pages > 1 && (
        <div className="flex gap-1.5 justify-center mt-6">
          {Array.from({ length: pages }, (_, i) => i + 1).map(pg => (
            <Link
              key={pg}
              href={`/online?sort=${sort}&q=${encodeURIComponent(search)}&p=${pg}`}
