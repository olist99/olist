...
export default async function MarketplacePage({ searchParams }) {
  ...
  async function buyAction(formData) {
    ...
    // Whitelist the currency column — NEVER interpolate user-controlled data
    const col = safeCurrencyColumn(listing.currency);
    if (!col) redirect('/marketplace?error=Invalid+currency');

    // Atomic deduct: only succeeds if buyer has enough
    ...
    // Transfer item
    if (listing.item_id) {
      await db('UPDATE items SET user_id = ?, room_id = 0 WHERE id = ?', [uid, listing.item_id]);
    }
  ...
  async function cancelAction(formData) {
    ...
    const listingId = (await import('@/lib/security')).safeInt(formData.get('listing_id'), 1);
    if (!listingId) redirect('/marketplace?error=Invalid+listing');
